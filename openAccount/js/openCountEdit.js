	...
	function nextStep(){
  		if(curr!=null){
  			...
  			if(curr==1){
  				...
  				if($("#openBasicAcc #accountType").html()!=null){
  		  			...
  		  			//load step2 
		  				...
		  				step2init();
  		  				...
  		  				$.get("step3.txt",function(data){
  		  					...
  		  					mydata!=""?show(mydata,"#divstep3"):"";
  		  					step3init();
  		  					
  		  					//加载协议链接
  		  					$("#PD1").next("a").attr("href",path+"CCSSA005.html");
  		  					$("#PD2").next("a").attr("href",path+"CCSSA006.html");
  		  					$("#PD3").next("a").attr("href",path+"paymentCardBack.html");
  		  					$("#PD4").next("a").attr("href",path+"CCSSA008.html");
  			...
  			if(curr==2){
  				...
  				if(countA<1){
  					...
  					$.get("window.txt",function(data){
  						...
  						mydata!=""?show(mydata,"#PD1Div"):"";
  						mydata!=""?show(mydata,"#PD3Div"):"";
  						windowinit();
...
/*		//产品开办窗口控制
		...
		$("#openBasicAcc #fileNumber").next("span").find("input").blur(function(){
		//	alert(1);
			var href= "testDemo://"+$("#openBasicAcc #fileNumber").textbox("getValue");
			$("#openBasicAcc #getLink").attr("href",href);
			//alert(href);
		});
		loading.hide();
	})
...
function windowinit(){
	...
	function step3init(){
		...
		if(vChoseProducts!=""&&vChoseProducts!=null){
			...
			for(var i=0; i<choseProducts.length; i++){
				...
				var data = $("#pageChose").combobox("getData");
				if(choseProducts[i]=="PD1"){
					data.push({ "text": "中银单位结算卡开办申请表", "value": "12" });
					$("#pageChose").combobox("loadData",data);
				}else if(choseProducts[i]=="PD2"){
					data.push({ "text": "对公短信通产品开办须知", "value": "151" });
					$("#pageChose").combobox("loadData",data);
				}else if(choseProducts[i]=="PD3"){
					data.push({ "text": "青岛市分行自助卡申请表", "value": "13" });
					$("#pageChose").combobox("loadData",data);
				}
			}
			...
			ajax.setSuccessCallback(function(data) {
				...
				if(data.success){
					...
					if (data.data){
						...
						mydata=data.data;
						$("#partner").textbox("setValue",data.data.clientInfo["partner"]);
						flag=false;
						//load step2 
						$.get("step2.txt",function(data){
							$("#divstep2").css("display","block");
	...
	function saveBook(sts,direct) {
		...
		if($("#products").length<1){
			...
			//开办产品选择项
			var choseProducts = "";
			$("#PD1Div .business_ss-dm input[id],#PD3Div .business_ss-dm input[id]").each(function(){
				var name = $(this).attr('id');
				dataJson[name] = "";
			});
		...
		if(dataJson["clientInfo.legalRepresentSurname"]!=null){
...
function show(data,id){
		...
		if (data){
			$(id+" .business_ss-dm input[id]").each(function(){
					var name = $(this).attr('id');
					if(name=="clientInfo.legalRepresentSurname"){
						if(data.linkMan!=null&&data.linkMan.length>0){
							$(this).textbox('setValue',data.linkMan.substring(0,1));
						}
					}else if(name=="clientInfo.legalRepresentName"){
						if(data.linkMan!=null&&data.linkMan.length>1){
							$(this).textbox('setValue',data.linkMan.substring(1,data.linkMan.length));
						}
					}else if(name=="depositorName"){
						if(data.accountName!=null&&data.accountName!=""){
							$(this).textbox('setValue',data.accountName);
						}
					}else if(name=="areaTax"){
						if(data.areaTax!=null&&data.areaTax!=""){
							$(this).textbox('setValue',data.areaTax.replace(/[\u4E00-\u9FA5]/g,''));
						}
					}else if(name=="countryTax"){
						if(data.countryTax!=null&&data.countryTax!=""){
							$(this).textbox('setValue',data.countryTax.replace(/[\u4E00-\u9FA5]/g,''));
						}
					}else if(name=="peopleType"||name=="registMoneyType"||name=="idType"){//由于下用的 combox 组合 input 
						if((data)[name]!=null&&(data)[name]!=""){
							$(this).combobox('setValue',(data)[name]);
						}  
					}else if(name.indexOf("\.")>0){
						var nameArr = name.split("\.");
						if(nameArr[1]=="nationalContact"||nameArr[1]=="nationalMoney"||nameArr[1]=="nationalLegal"){
							if(data[nameArr[0]]!=null){
								$(this).textbox('setValue',data[nameArr[0]][nameArr[1]]==null?"CN":data[nameArr[0]][nameArr[1]]);
							}
						}else if(data[nameArr[0]]!=null&&data[nameArr[0]][nameArr[1]]!=null&&data[nameArr[0]][nameArr[1]]!=""){
							$(this).textbox('setValue',data[nameArr[0]][nameArr[1]]);
						}
						
					}else{
					
						if((data)[name]!=null&&(data)[name]!=""){
							$(this).textbox('setValue',(data)[name]);
						} 
						
					}
			});
			$(id+" .business_ss-dm select[id]").each(
				function(){
					var name = $(this).attr('id');
					if(name.indexOf("\.")>0){
						var nameArr = name.split("\.")
						if(data[nameArr[0]]!=null&&data[nameArr[0]][nameArr[1]]!=null&&data[nameArr[0]][nameArr[1]]!=""){
							$(this).combobox('setValue',data[nameArr[0]][nameArr[1]]);
						}
					}else{
						if((data)[name]!=null&&(data)[name]!=""){
							$(this).combobox('setValue',(data)[name]);
			...
			$("#chargeDiv .business_ss-dm select[id]").each(
				...
				//	console.log("charge:"+name);
					...
					if(data.bookCharge!=null&&(data.bookCharge)[name]!=null&&(data.bookCharge)[name]!=""){
						$(this).combobox('setValue',(data.bookCharge)[name]);
					}
			});
			//核对框入局填充
	/*		$("#basicInfoCheckW td[tdName]").each(
				function(){
					...
					names=name.split(".");
					if(names.length>1){
						if(data.bookCharge!=null&&(data.bookCharge)[names[1]]!=null){
							$(this).text((data.bookCharge)[names[1]]);
						}
					}else{
						if(data!=null&&(data)[name]!=null){
							if(name=="registAddress"){
								$(this).text((data)[name].replace(/[,]/g,""));
							}else{
								$(this).text((data)[name]);	
							}
						}
